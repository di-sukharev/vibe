...
- Worker: `bun run start:worker`, backed by `src/worker.ts`. A loop over `workerLoops`, for work that must run more often than once a minute. Ships empty, and deployment generation refuses to deploy it as an App Platform worker until it has entries.
